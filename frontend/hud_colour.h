...
#ifndef INC_HUD_COLOUR_H_
#define INC_HUD_COLOUR_H_
...
public:
	...
	static void		SetValuesFromDataFile();
	static CRGBA	GetRGBA(eHUD_COLOURS colour_id);
	static CRGBA	GetRGB(eHUD_COLOURS colour_id, u8 alpha);
	static CRGBA	GetRGBF(eHUD_COLOURS colour_id, float fAlphaFader);
	static void		GetRGBValues(eHUD_COLOURS colour_id, int& r, int& g, int& b);
	static u32		GetIntColour(eHUD_COLOURS colour_id);

	static void		SetRGBAValue(eHUD_COLOURS colour_id, s32 r, s32 g, s32 b, s32 a);
	static void		SetIntColour(eHUD_COLOURS colour_id, u32 colour);

	static void		SetNameRef(eHUD_COLOURS colour_id, u32 iKey) {m_NameRef[colour_id] = iKey; }
	static char		*GetNameString(eHUD_COLOURS const c_colourId);

	static atHashString	GetKeyFromColour(eHUD_COLOURS const c_colorId);
	static eHUD_COLOURS	GetColourFromKey(atHashString ColourName);
	static eHUD_COLOURS	GetColourFromRGBA(Color32 const c_rgba);

	static bool IsHudColourRed(eHUD_COLOURS colour) { return colour >= HUD_COLOUR_RED && colour <= HUD_COLOUR_REDDARK; }

	static bool IsInitialized() { return bHudColoursInitialised; }
private:
	static bool		bHudColoursInitialised;
	static atRangeArray<Color32,HUD_COLOUR_MAX_COLOURS>	m_values;  // all the HUD colours
	static atRangeArray<u32,HUD_COLOUR_MAX_COLOURS>		m_NameRef;
};

#endif // INC_HUD_COLOUR_H_
