...
#ifdef NVSTEREO
	float4	ScreenPosStereo	: TEXCOORD6;
#endif
...
#define PSBlendFactor		Params4.y
#define PSFlowVector		Params4.zw
};

VS_OUTPUT	VS_RiverShallowDepth(VS_INPUT IN)
{
	VS_OUTPUT	OUT;
...
#else
	OUT.PSFlowVector		= float2(0.0f,0.0f);
...
float4	PSUnderwaterCommon(VS_OUTPUT IN, uniform bool UseParaboloidMap)
{
	...
	float4 waterColor = UnderwaterCommon(
			...
			IN.PSBlendFactor,								//ReflectionBlend
			IN.PSNormal,									//Normal
...
wfloat4	RiverCommon(VS_OUTPUT IN, uniform bool HighDetail, uniform bool UseFogPrepass)
{
	...
	if(UseFogPrepass)
		reflectionBlend = IN.PSBlendFactor;
	else
		reflectionBlend = 1;
...
half4 PS				(VS_OUTPUT IN) : COLOR	{ return RiverCommon(IN, true, true); }
half4 PS_SinglePass		(VS_OUTPUT IN) : COLOR	{ return RiverCommon(IN, false, false); }
...
technique clouddepth_draw
{
	pass p0
	{
		...
		VertexShader		= compile VERTEXSHADER	VS_RiverShallowDepth();
#if __XENON
		PixelShader			= NULL;
#else
		ColorWriteEnable	= 0;
