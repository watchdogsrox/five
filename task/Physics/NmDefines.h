// Filename   :	NmDefines.h   
// Description:	Enumerations defining the interface to the Natural Motion behaviours and parameters (included by TaskNM.h).

#ifndef INC_NMDEFINES_H_
#define INC_NMDEFINES_H_

// This must match the enum at the top of NmParameterSets.cpp
enum eEffectorMaskStrings
{
	NM_MASK_FULL_BODY = 0,
...
enum eRagdollTriggerTypes
{
	RAGDOLL_TRIGGER_BULLET = 0,
	...
	RAGDOLL_TRIGGER_SNOWBALL,
	RAGDOLL_TRIGGER_NUM
};

// NB - If changing these message definitions, remember to update nm_strings.sch or scripts will fail!
enum eNMStrings
{
	NM_STOP_ALL_MSG = 0,
...
#define NM_NUM_MESSAGES (90)

enum eBlendFromNMOptions
{
	BLEND_FROM_NM_INVALID = 0,
	BLEND_FROM_NM_ANIM,
	BLEND_FROM_NM_GETUP,
	BLEND_FROM_NM_GETUP_FAST,
	BLEND_FROM_NM_STANDING,
	BLEND_FROM_NM_TO_INJURED,
	BLEND_FROM_NM_REVIVE,
	BLEND_FROM_NM_SWIMING,
	BLEND_FROM_NM_CLIMBING,
	BLEND_FROM_NM_SCRIPT
};


// A small class to hold:
//    * the static strings which the enumerations above dial into to actually access the NM behaviours/parameters.
//    * the static strings which define the effector masks for various NM behaviours.
class CNmDefines
{
public:

	friend class CTaskNMBehaviour;
	friend class CClipPoseHelper;

	CNmDefines(){}
	~CNmDefines(){}

private:
	static eNMStrings ms_nMessageList[NM_NUM_MESSAGES + 1];
	static const char* ms_aNMStrings[NM_NUM_STRINGS + 1];
	static const char ms_aNMMaskStrings[NM_NUM_MASK_STRINGS + 1][3];

#if __ASSERT
public:
	static const char* ms_aNMComponentNames[21];
#endif // __ASSERT

#if !__NO_OUTPUT
	static const char* ms_aRagdollTriggerTypeNames[RAGDOLL_TRIGGER_NUM+1];
#endif //!__NO_OUTPUT
};

#endif // !INC_NMDEFINES_H_
