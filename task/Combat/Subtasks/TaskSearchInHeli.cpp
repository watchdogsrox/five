...
CTaskSearchInHeli::Tunables CTaskSearchInHeli::sm_Tunables;
IMPLEMENT_COMBAT_TASK_TUNABLES(CTaskSearchInHeli, 0x9410823d);

CTaskSearchInHeli::CTaskSearchInHeli(const Params& rParams)
: CTaskSearchInVehicleBase(rParams)
{
	SetInternalTaskType(CTaskTypes::TASK_SEARCH_IN_HELI);
}

CTaskSearchInHeli::~CTaskSearchInHeli()
{
}

#if !__FINAL
const char* CTaskSearchInHeli::GetStaticStateName(s32 iState)
{
...
CTask::FSM_Return CTaskSearchInHeli::UpdateFSM(const s32 iState, const FSM_Event iEvent)
{
	FSM_Begin
...
CTask::FSM_Return CTaskSearchInHeli::Start_OnUpdate()
{
...
void CTaskSearchInHeli::Search_OnEnter()
{
	...
	params.m_iDrivingFlags = DF_DontTerminateTaskWhenAchieved|DF_AvoidTargetCoors;
	params.m_fTargetArriveDist = 0.0f;
	params.m_fCruiseSpeed = sm_Tunables.m_CruiseSpeed;
	
	//Create the vehicle task.
	CTask* pVehicleTask = rage_new CTaskVehicleGoToHelicopter(params, 0, -1.0f, sm_Tunables.m_MinHeightAboveTerrain);
...
CTask::FSM_Return CTaskSearchInHeli::Search_OnUpdate()
{
